...
export function AddCategory({ open, close, refetch }: Props) {
  ...
  const { mutate: add, isLoading: addLoad } = useMutation({
    mutationFn: (data: CategoryResponse) => categoryService.create(data),
    onSuccess: () => {
  ...
  const { mutate: edit, isLoading: editLoad } = useMutation({
    mutationFn: (data: CategoryResponse) => categoryService.update(open?.id, data),
    onSuccess: () => {
          ...
          <ModalHeader p="26px 30px 15px 30px">
            <HStack justify="space-between">
              <Text fontSize="30px" fontWeight="700">
                {open?.id ? "Редактировать категорию" : 'Добавить категорию'}
              </Text>
              <LanguageChange />
            </HStack>
          </ModalHeader>
